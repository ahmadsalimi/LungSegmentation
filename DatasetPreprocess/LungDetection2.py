...
class MaskPreprocessor(object):
    ...
    def make_lungmask(self, img, nodules_mask, threshold, mean, std, img_name, si, relative_loc):
        ...
        bnd = np.logical_not(o_mask) & morphology.dilation(o_mask, np.ones([10, 10]))
        bnd[:10, :] = True
        bnd[-10:, :] = True
        bnd[:, :10] = True
        bnd[:, -10:] = True
            ...
            try:
                ...
                for lbl in [-1, -2]:
                    ...
                    for _ in range(3):
                        ...
                        msk_cpy = morphology.dilation(msk_cpy, np.ones([16, 16]))
                        msk_cpy = morphology.erosion(msk_cpy, np.ones([16, 16]))

                    msk_cpy = morphology.erosion(msk_cpy, np.ones([5, 5]))
