...
class TableLoader(ContentLoader):
    ...
    def load_tables(self, data_specification):
        ...
        valid_id = np.vectorize(lambda x: x in self.conf['LabelMapDict'])
        ids = ids[valid_id(ids[:, 0]), :]

        views_base_name_ids = np.unique(np.vectorize(lambda x: x[:x.rfind('_')])(ids[:, 1]))

        indices = self.get_reordered_indices(self.data['ID'], views_base_name_ids)

        self.specific_table = self.data.loc[indices]
        self.specific_table.set_index(np.arange(len(indices)), inplace=True)

    def get_samples_names(self):
        return self.specific_table['ID']

    def get_samples_labels(self):
        return self.specific_table['Label']
    ...
    def reorder_samples(self, samples_names):
        indices = self.get_reordered_indices(self.specific_table['ID'], samples_names)
    ...
    def fix_multiple_views(self, views_dict, views_names):
        df = pd.DataFrame(np.repeat(self.specific_table.values, views_dict, axis=0),
                          columns=self.specific_table.columns.values)
        self.specific_table = df
        self.specific_table['ID'] = np.asarray(views_names)
    ...
    def get_table_features(self, batch_chooser):
        if self.conf['augment']:
            if self.conf['phase'] == 'train':
                if self.data_specification == 'train':
                    if self.conf['binary_augment']:
                        return self.binary_augment_add(
                            self.specific_table.iloc[batch_chooser.get_current_batch_sample_indices(),
                            1:].values.astype(
                                np.float64))
                    else:
                        return self.augment_add(
                            self.specific_table.iloc[batch_chooser.get_current_batch_sample_indices(),
                            1:].values.astype(
                                np.float64))
                else:
                    return np.repeat(self.specific_table.iloc[batch_chooser.get_current_batch_sample_indices(),
                                     1:-1].values.astype(
                        np.float64), self.conf['repeat_value'], axis=0)
            elif self.conf['phase'] == 'eval':
                return self.binary_augment_add(
                    self.specific_table.iloc[batch_chooser.get_current_batch_sample_indices(), 1:].values.astype(
                        np.float64))
        else:
    ...
    def get_non_essential_features(self, batch_chooser):
        return self.specific_table.iloc[batch_chooser.get_current_batch_sample_indices(), 3:-1].values.astype(
            np.float64)

    def get_life_status(self, batch_chooser):
        if self.conf['augment']:
            if self.conf['phase'] == 'train':
                return np.repeat(self.specific_table.iloc[
                    batch_chooser.get_current_batch_sample_indices(), -1].values.astype(
                    np.float64), self.conf['repeat_value'])

            if self.conf['phase'] == 'eval':
                return np.repeat(self.specific_table.iloc[
                    batch_chooser.get_current_batch_sample_indices(), -1].values.astype(
                    np.float64), self.conf['repeat_value'])
        else:
    ...
    def set_views_names(self, views_names):
        self.specific_table['ID'] = views_names
